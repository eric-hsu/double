 ...
 */
@Service
public class PabBuckleCapitalQueryService extends PabAbstractService {
	...
	public String perform(Map<String, Object> params) {
		...
		logger.info("**************************平安代发代扣结果查询接口[4048]***************************");
		logger.info("代发代扣结果查询交易请求报文："+JSON.toJSONString(params));
    	 ...
    	 * 1,请求数据合法性验证
    	 */
  		OperateResult operateResult = CheckPabRequestParam.checkcEnterpriseBuckleCapitalQueryParam(params);
		...
		if(tradeRecordItem.getItemStatus()==1){
			String returnJsonData = PabHandle.createPabBuckleCapitalQueryResultJson(tradeRecordItem);
		    returnInfo.setSuccess(true);
		    returnInfo.setErrormsg("");
		    returnInfo.setData(returnJsonData);
		    logger.info("代发代扣结果查询,结果"+JSON.toJSONString(returnInfo)+"");
		    return JSON.toJSONString(returnInfo)+"";
		}
		/**
    	 * 3,发送数据报文组装
    	 */
        String sendxml = PabHandle.getEnterpriseBuckleCapitalQueryXml(tradeRecordItem);
        params.put("xmllength", String.valueOf(sendxml.getBytes("gbk").length));
        params.put("trade_code", PabConstants.trade_code4048);
        ...
        logger.info("代发代扣结果查询交易网关查询发送报文：【"+senddata+"】");
        
        /**
    	 * 4,发送银行网关，并同步获取返回结果
    	 */
        String returnstr = PabHttprequest.send(senddata);
        logger.info("代发代扣结果查询交易网关查询返回报文：【"+returnstr+"】");
        if(returnstr==null || "".equals(returnstr)){
        	returnInfo.setErrormsg(Constants.GATEWAY_ERROR);
  			logger.info("代发代扣结果查询接口[4048]网关返回数据为空:"+JSON.toJSONString(returnInfo));
  			return  JSON.toJSONString(returnInfo);
        }
        ...
        if(!PabConstants.GATEWAY_SUCCESS_CODE.equals(returnCode)){
        	returnInfo.setErrormsg(returnMsg);
  			logger.info("代发代扣结果查询接口[4048]网关返回错误，错误信息:"+JSON.toJSONString(returnInfo));
  			return  JSON.toJSONString(returnInfo);
        }
	    ...
	    returnInfo.setErrormsg("");
        returnInfo.setData(returnJsonData);
		logger.info("代发代扣结果查询同步返回报文："+JSON.toJSONString(returnInfo));
    	} catch (Exception e) {
			e.printStackTrace();
