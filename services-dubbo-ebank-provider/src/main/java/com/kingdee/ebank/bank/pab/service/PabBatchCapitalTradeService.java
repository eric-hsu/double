...
import com.kingdee.ebank.bank.pab.util.EnterpriseBatchCapitalTradeResult;
 ...
 */
@Service
public class PabBatchCapitalTradeService extends PabAbstractService {
	...
	public String perform(Map<String, Object> params) {
		...
		logger.info("**************************平安企业大批量资金划转[4018]***************************");
		logger.info("企业大批量资金划转交易请求报文："+JSON.toJSONString(params));
		...
		TradeRecord tradeRecordNew = PabHandle.createEnterpriseBatchCapitalTradeVo(params);
		tradeRecordService.add(tradeRecordNew);
		logger.info("企业大批量资金划转订单记录已保存，记录id["+tradeRecordNew.getId()+"]");
		...
		logger.info("企业大批量资金划转交易记录已保存.");
		

		/**
    	 * 4,发送数据报文组装
    	 */
        String sendxml = PabHandle.getEnterpriseBatchCapitalTradeXml(tradeRecordNew);
        ...
        if(!PabConstants.GATEWAY_SUCCESS_CODE.equals(returnCode)){
        	returnInfo.setErrormsg(returnMsg);
  			logger.info("企业大批量资金划转网关返回错误，错误信息:"+JSON.toJSONString(returnInfo));
  			return  JSON.toJSONString(returnInfo);
        }
        ...
        EnterpriseBatchCapitalTradeResult refundResult = JaxbMapper.fromXml(returnxml, EnterpriseBatchCapitalTradeResult.class);
    	 ...
    	 * 7, 同步返回产品线
    	 */
        String returnJsonData = PabHandle.createEnterpriseBatchCapitalTradeResultJson(tradeRecordNew);
        returnInfo.setSuccess(true);
	    returnInfo.setErrormsg("");
        returnInfo.setData(returnJsonData);
		logger.info("企业大批量资金划转交易同步返回报文："+JSON.toJSONString(returnInfo));
    	} catch (Exception e) {
			e.printStackTrace();
